...
from torch_geometric.nn import GINConv, global_add_pool, global_mean_pool, global_max_pool, GATConv, GCNConv
...
class GraphNorm(nn.Module):
    ...
    def forward(self, node_emb, graph):
        ...
        # Calculate the number of unique graphs in the batch
        batch_size = graph.batch.max().item() + 1

        node_mean = scatter(node_emb, graph.batch, dim=0, dim_size=batch_size, reduce='mean')
        node_mean = node_mean.repeat_interleave(num_nodes_list, 0)

        sub = node_emb - node_mean * self.scale
        node_std = scatter(sub.pow(2), graph.batch, dim=0, dim_size=batch_size, reduce='mean')
...
class GIN(torch.nn.Module):
    ...
    def __init__(self, dim_features, hidden_dim, dim_targets, use_norm, num_layers, pooling):
...
class OCGTL(nn.Module):
    def __init__(self, dim_features, config):
        ...
        num_trans = config.get('num_trans', 6)
        hidden_dim = config.get('hidden_dim', 16)
        dim_targets = config.get('dim_targets', 16)
        num_layers = config.get('num_layers', 2)
        use_norm = config.get('use_norm', 'gn')
        pooling = config.get('pooling', 'add')
        
        self.device = 'cuda' if torch.cuda.is_available() else 'cpu'
        self.gins = []
        for _ in range(num_trans):
            self.gins.append(GIN(dim_features, hidden_dim, dim_targets, use_norm, num_layers, pooling))

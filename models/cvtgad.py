...
from torch_geometric.nn import GINConv, GCNConv, GATConv, global_mean_pool, global_max_pool
import torch
...
class CVTGAD(nn.Module):
    def __init__(self, feat_dim, config):
        ...
        num_gc_layers = config.get('num_gc_layers', 2)
        str_dim = config.get('rw_dim', 16) + config.get('dg_dim', 16)
        gnn_layer = config.get('gnn_layer', 'GINConv')
        graph_level_pool = config.get('graph_level_pool', 'global_mean_pool')
        self.embedding_dim = hidden_dim * num_gc_layers

        if gnn_layer == 'GCNConv':
            self.encoder_feat = Encoder_GCN(feat_dim, hidden_dim, num_gc_layers, graph_level_pool)
            self.encoder_str = Encoder_GCN(str_dim, hidden_dim, num_gc_layers, graph_level_pool)
        elif gnn_layer == 'GINConv':
            self.encoder_feat = Encoder_GIN(feat_dim, hidden_dim, num_gc_layers, graph_level_pool)
            self.encoder_str = Encoder_GIN(str_dim, hidden_dim, num_gc_layers, graph_level_pool)
        elif gnn_layer == 'GATConv':
            self.encoder_feat = Encoder_GAT(feat_dim, hidden_dim, num_gc_layers, graph_level_pool)
            self.encoder_str = Encoder_GAT(str_dim, hidden_dim, num_gc_layers, graph_level_pool)
...
class Encoder_GIN(torch.nn.Module):
    def __init__(self, num_features, dim, num_gc_layers, graph_level_pool='global_mean_pool'):
        ...
        self.pool_type = graph_level_pool

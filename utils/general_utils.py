...
class EarlyStopper:
    def stop(self, epoch, train_loss=None, val_auc=None):
        raise NotImplementedError("Implement this method!")

    def is_best(self):
        return self.is_best_epoch

    def get_best_vl_metrics(self):
        return self.train_loss, self.val_auc, self.best_epoch
...
class Patience(EarlyStopper):
    ...
    def stop(self, epoch, train_loss=None, val_auc=None):
        if self.use_train_loss:
            ...
            current_metric = -val_auc  # Negative because we want to maximize val_auc

        if current_metric <= self.local_val_optimum:
            self.counter = 0
            self.local_val_optimum = current_metric
